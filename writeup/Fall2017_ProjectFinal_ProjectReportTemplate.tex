...
Training was done with filtered data from \textit{framesdataset}. We cut out a $15\times40$ patch for each eye in each frame using the location reported by \textit{clmtrackr}. Each eye was greyscaled, zero-meaned, and scaled into units of standard deviations of each batch (size 50). The CNN for each eye is trained for 100 epochs with a learning rate of 0.0001 using gradient descent.
