...
with open( dirToView + dataFile ) as f:
    ...
    for row in readCSV:
        ...
        for i in range(0,28,2):
            img = cv2.line(img, (clmTrackerInt[i],clmTrackerInt[i+1]), (clmTrackerInt[i+2],clmTrackerInt[i+3]), (0,255,0), 4)
        ...
        for i in range(0,len(clmTrackerInt)-1,2):
            img = cv2.circle(img, (clmTrackerInt[i],clmTrackerInt[i+1]), 2, (0,0,255), -2 )
